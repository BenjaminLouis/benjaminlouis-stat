---
title: 'PDF vs HTML : Extract information from a Google Form with R'
...
As I cannot publicly share the orginal form, I made a dummy one from my Google account by mixing proposed templates. The `PDF` file can be easily downloaded from the Google Form account. For `HTML`, I first wanted to directly reach the page from the editable version of the form using the [rvest](https://blog.rstudio.com/2014/11/24/rvest-easy-web-scraping-with-r/) package but I never succeeded as it seems Google double authentification makes things difficult. If somebody with better skills for this kind of problem have a solution, I'll take it ! Instead of performing classic webscraping, I saved the `HTML` page from my browser and worked with the file locally. All materials are available [here](https://github.com/BenjaminLouis/blog_materials/tree/master/2019-03-29-pdf-vs-html).
...
This function was perfect for this work as the returned format is nearly the same as the one I wanted. To really obtained the final format, I just bound all the data frames by row being careful of keeping a pages identifier.
...
That's it ! With only one function used once, all the wanted data were extracted from the `PDF`. The rest is "just" data wrangling. At this point, I was convinced that the work was nearly finished but tidying the data frame meant finding some rules to get the three wanted columns from the ones returned by the function. And finding these rules meant a permanent look at the `PDF` while tidying the data frame.
...
The second step was to identifying "sentences" among lines. The idea here was that a sentence starts either by one or two digits following by a dot and a space (question titles) or by an uppercase letter following by a lowercase letter or a space (answer choices). If a question title or an answer choice takes place on two lines of the `PDF`, I'll be able to consider both lines in the same sentence. In order to do that, I created a `sentences_group` column by a) detecting each text box starting a sentence using [regular expressions](https://stringr.tidyverse.org/articles/regular-expressions.html), b) applying a cumulative sum on the boolean so each line belonging to the same sentence have the same sum and c) transforming to character by adding a prefix. This column helped me collapsing sentences and removing duplicated ones.
...
For each question bloc, I was able to get the questions' type thanks to the default behavior of Google Forms adding a sentence beside each question title to specify how many answer can be given. These sentences are then removed and an identifier is given for sentences in each question bloc.
...
If you have never heard about `HTML` and `CSS`, maybe what comes after will be a bit obscure but I think this is unlikely. Content of a web page is stored in `HTML` page and webscraping is to collect these data. This is why the [rvest](https://blog.rstudio.com/2014/11/24/rvest-easy-web-scraping-with-r/) package was made for and I obviously used it for this work.

### rvest, selectorGadget and CSS selector

Basically, a `HTML` page has a tree structure (called a DOM for _Document Object Model_) where each node contains objects. Function `read_html` enables to read and import `HTML` page into R.
